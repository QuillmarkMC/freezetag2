...
$execute if data storage freeze:menu {TempEnabled:"true"} run function freeze:lobby/menu/item_options/edit/disable {Page:$(Page),NumItem:$(NumItem),Name:"$(Name)"}
$execute if data storage freeze:menu {TempEnabled:"false"} run function freeze:lobby/menu/item_options/edit/enable {Page:$(Page),NumItem:$(NumItem),Name:"$(Name)"}
