...
#Use
execute store result score @s item_giver_cooldown run data get entity @s item.components.minecraft:custom_data.Uses
scoreboard players remove @s item_giver_cooldown 1
execute store result entity @s item.components.minecraft:custom_data.Uses int 1 run scoreboard players get @s item_giver_cooldown
execute if score @s item_giver_cooldown matches 0 run function freeze:game/general/items/mystery/temp_item_giver/clean with entity @s item.components.minecraft:custom_data
